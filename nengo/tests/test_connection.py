...
def test_pes_learning_rule(Simulator, nl_nodirect):
    ...
    with m:
        ...
        u_learned_p = nengo.Probe(u_learned, filter=0.1)
        e_p = nengo.Probe(e, filter=0.1)
...
def test_voja_learning_rule(Simulator, nl_nodirect):
    ...
    def sim_encoders(sim, x):
        """Returns the sim's current non-scaled encoders for Ensemble x."""
        scaled_encoders = sim.signals[sim.model.sig_scaled_encoders[x]]
        return scaled_encoders / (
            sim.data[x.neurons].gain / x.radius)[:, np.newaxis]
