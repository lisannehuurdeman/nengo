...
class NengoObjectContainer(type):
    ...
    are some differences with how this work in this class, so they are
    ...
    ``with self`` inside a Network subclass's __init__.
...
class NetworkMember(type):
    ...
    If add_to_network=False is passed the base class, then the current Network
    will not be determined and the callback will not be invoked.
    """
    def __call__(cls, *args, **kwargs):
        """Override default __call__ behavior.

        type.__call__ is what gets invoked when you do type(cls, bases, dict).
        By default, type.__call__ calls the base class's __new__,
        and then __init__, as long as __new__ returns an instance of the
        correct class.

        This overrides the default type.__call__ in order to do all of the
        add_to_network stuff automatically after __init__.
        This allows us to reduce boilerplate while allowing
        the network members to look like normal Python classes.
        """
        inst = cls.__new__(cls)
        if not isinstance(inst, cls):
            return inst
